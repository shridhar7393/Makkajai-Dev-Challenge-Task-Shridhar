...
public class SalesTaxUnitTest {
	...
	@Test
	public void testWhetherInputStringIsConvertedToListOfItems() {
		...
		for(Item item: aList) {
			...
			assertEquals("47.50", sc.roundUpToTwoDecimals(item.getPrice()).toString());
	...
	@Test
	public void inputFileIsConvertedToItemsListwithValues() {
	...
	@Test
	public void testOutputFromInput1() {
		...
		for (Item item : items) {
			...
			assertEquals("1.50", stc1.roundUpToTwoDecimals(rc.getTotalSalesTax()).toString());
			assertEquals("29.83", stc1.roundUpToTwoDecimals(rc.getTotalCost()).toString());
		...
		@Test
		public void testOutputFromInput2() {
			SalesTaxCalculator stc1 = new SalesTaxCalculator();
			Receipt rc = new Receipt();
			...
			for (Item item : items) {
				...
				assertEquals("7.65", stc1.roundUpToTwoDecimals(rc.getTotalSalesTax()).toString());
				assertEquals("65.15", stc1.roundUpToTwoDecimals(rc.getTotalCost()).toString());
		...
		@Test
		public void testOutputFromInput13() {
			SalesTaxCalculator stc1 = new SalesTaxCalculator();
			Receipt rc = new Receipt();
			...
			for (Item item : items) {
				...
				assertEquals("6.70", stc1.roundUpToTwoDecimals(rc.getTotalSalesTax()).toString());
				assertEquals("74.68", stc1.roundUpToTwoDecimals(rc.getTotalCost()).toString());
